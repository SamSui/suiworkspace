...
async def test_rerank_records_metric():
    metrics.reset()
    chunks = [
        {"chunk_id": "a", "doc_id": "", "kb_id": "", "score": 0.3, "source": "vector", "highlight": None},
        {"chunk_id": "b", "doc_id": "", "kb_id": "", "score": 0.9, "source": "vector", "highlight": None},
    ]
...
async def test_retrieve_cache_hit_skips_recall():
    ...
    metrics.reset()
    cached = [{"chunk_id": "pre", "doc_id": "", "kb_id": "k1", "score": 1.0, "source": "vector", "highlight": None}]
    ...
    settings = type("S", (), {"retrieval": type("R", (), {"embed_dim": 8, "embed_provider": "echo", "vector_top_k": 30, "keyword_top_k": 30, "rerank_top_k": 5, "rerank_provider": "echo", "cache_ttl_seconds": 10})(), "redis": type("RS", (), {"prefix_retrieval_cache": "cache:ret:"})()})()

    cfg = {"configurable": {"deps": {"container": container, "settings": settings}}}
    upd = await retrieve_node(
        {"query": "报销", "kb_id": "k1"}, cfg
    )

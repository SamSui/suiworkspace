...
- **收尾回执「小步降载」（2026-09-25 加固）** → 在背景已堆积很多（大量文件引用、长评论链）的高语境 issue 里，把「收尾 / 交付回执」做成精简的「先结论后细节」：先发最终结论与证据摘要，再按需补次要佐证，绝不把关键回执塞进超长单轮。原因：增量交付收尾阶段（2026-09-24）又现一次 token 超限（请求 263271 / 上限 262144），再叠加网关 504 上游超时，「任务都完成了吗」被重新追问才补齐回执，宏盘与验收悬置、往返放大。后果：把 owner 想要的「完成回执」稳定送出去，降低 context/504 打断导致回执丢失的概率，验收推进更快。

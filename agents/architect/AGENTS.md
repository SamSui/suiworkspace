...
- **⑤ 高语境 issue「每轮降载 gate」（2026-09-25 加固）** → 上下文预算 gate **不只用于派发长任务前**：对已累积大量文件引用 / 长评论链的高语境 issue，**每一轮参与（回复、验收判断、回执、评审回签）前都做一次 context 预算自检**，若接近容量先发结论、再按需补细节；并在派发言中提示执行器「收尾 / 交付回执也要先结论后细节，精简降载」。缘由：增量 5 收尾（2026-09-24）再现 token 超限（请求 263271 / 上限 262144）叠加网关 504，「任务都完成了吗」需二次追问才补齐回执，宏盘与验收悬置。后果：把 owner 的「完成回执」在顺畅通道稳定交付，降低 context/504 丢回执频率，缩短验收往返。

...
## 验收与委托规范（复盘落地 · 2026-09-23，owner 随祥熙授权）

> 以下三条由 2026-09-23 突击小队复盘提炼、随祥熙确认「按照建议事项优化」后纳入，作为阶段验收与任务委托的硬规范。
...
- **要求执行器每完成一个步骤即穿插中途 CHECKPOINT**，防止单 turn 拉爆上下文（本轮随研发 2.1 初委托即因 context_overflow 中断无产出）。
- **运行/环境依赖授权前置**：若做运行时验证涉及 `pip install`、起 Docker 等需授权动作，在任务描述里一并预置隔离的可执行方案（如项目内 .venv），并把「需要 owner 授权安装」当确认进去，而非等代码全部完成后再卡在权限上。
- 委托模板在 `docs/复盘优化落地-2026-09-23.md` 附回文本，可直接复用。

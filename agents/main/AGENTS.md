...
以下每项是**必交工件**（协调类是周期性主轴，缺一即记需补齐）：

- **任务登记** → 每条源于口头结论的任务都落成有归属、有状态、有记录的 Multica issue（含对象）。
- **跨角色依赖 / 资源矩阵** → 关键路径上角色间的依赖与阻塞清单（含卡点与负责人）。
- **跟进闭环纪要** → 对已决议事项的收敛记录（周度为周期有输出，任一句到底）。

**每日摘要数据源（复盘落地 · 2026-09-23，不得只读看板）**
- 每日进度/复盘摘要**不得只读看板 status**；须并入**运行级交付回执**作为「实际完成」的判定：任务附件的产出文件、仓库 commit / PR、运行级验证（冒烟/门禁）通过记录、评审通过结论。
- 映射到 agent 与 issue 编号 + 标题，标题对齐「昨日完成 / 今日推进 / 卡住」三块；某 agent 在窗口内仅有运行级证据而无看板状态变更，也应在摘要中体现，避免「摘要说没事、实际在等授权/冒烟」的错位。
- 只写真实状态，不推测、不编造；无实际工作如实标注「无信息可复盘」。
